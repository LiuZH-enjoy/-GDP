%%  输入原始数据并做出时间序列图
clear;clc
year =[2010:2020]';  % 横坐标表示term，写成列向量的形式（加'就表示转置）
% x0 = [412119.3	487940.2	538580	592963.2	643563.1	688858.2	746395.1	832035.9	919281.1	986515.2	1015986.2]';  %原始数据序列，写成列向量的形式（加'就表示转置）
x0 = [30808	36302	39874	43684	47173	50237	54139	60014	66006	70581	72447]'
...
xlabel('term');  ylabel('gpa');  % 给坐标轴加上标签
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    ...
    xlabel('term');  ylabel('原始数据的光滑度');  % 给坐标轴加上标签
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    if  n > 4  % 数据量大于4时，将数据分为训练组和试验组(根据原数据量大小n来取，n为5-7个则取最后两年为试验组，n大于7则取最后三年为试验组)
        ...
        test_year = year(end-test_num+1:end);  % 试验组对应的term
        figure(3)
        ...
        legend('real','traditional GM(1,1)','new inf GM(1,1)','metabolism GM(1,1)')  
        % 注意：如果lengend挡着了图形中的直线，那么lengend的位置可以自己手动拖动
        %'试验组的真实数据','传统GM(1,1)预测结果','新信息GM(1,1)预测结果','新陈代谢GM(1,1)预测结果'
        xlabel('term');  ylabel('gpa');  % 给坐标轴加上标签
        % 计算误差平方和SSE
    ...
    legend('Relative residuals'); xlabel('term');%相对残差
    set(gca,'xtick',year(2:1:end))  % 设置x轴横坐标的间隔为1
    subplot(2,1,2)
    plot(year(2:end), eta,'o-'); grid on;   % 原数据中的各时期和级比偏差
    legend('Grade ratio deviation'); xlabel('term');%极比偏差
    set(gca,'xtick',year(2:1:end))  % 设置x轴横坐标的间隔为1
    disp(' ')
    ...
    legend('basic data','fitted data','predicted data')  % 注意：如果lengend挡着了图形中的直线，那么lengend的位置可以自己手动拖动
    %原始数据，拟合数据，预测数据
